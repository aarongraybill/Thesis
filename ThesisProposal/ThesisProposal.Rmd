...
[^3]: [<https://en.wikipedia.org/wiki/List_of_countries_by_GDP_(nominal>)](https://en.wikipedia.org/wiki/List_of_countries_by_GDP_(nominal)) I need a source that isn't wikipedia, but maybe this is stupid anyway.
...
Platforms are fundamentally a marketplace. *Content creators*, firms, are given an opportunity to connect with consumers who wish to consumer their product. However, platforms provide a marketplace unlike anything that has been previously seen. One key distinction between a physical marketplace and a digital platform is that consumption is usually nonrivalrous. One consumer's decision to listen to an artist's newest release on Spotify has no negative impact on another consumer's ability to consume that product. A content creator's inventory is limited only by the bandwidth of the platform and the number of consumers who wish to consume their product. The number of willing consumers is a content creator's audience, and is central to the analysis conducted below.

Audience evolution is another key difference between a streaming economy and traditional market structures. Of course, consumers can seek out content directly if they know that a creator exists, but that does not fully describe consumer-creator matching on a platform. A central component of streaming markets is the *algorithm*. A platform's algorithm can take multiple forms, but I will use it mean a platform's way of analyzing a release by a content creator and deciding which consumers will be shown that release. In some markets, like an outdoor farmer's market, the algorithm is nonexistent. Consumers are required to self-select products they wish to purchase. However, platforms use their algorithm to show content to consumers content they may not have ex ante knowledge.

Content creators have both celebrated algorithms as a way of gaining exposure that would have been impossible in a pre-internet economy. Others have maligned the algorithm for its unpredictability and bias towards content that appeals to the lowest common denominator. However, all successful content creators must acknowledge the importance of a platform's algorithm and adapt to its changes.

A useful anecdote comes from Stevie Wynne Levine, Chief Creative Officer of Mythical. Mythical is a YouTube video conglomerate with over 75 million subscriber across its channels and 25 billion total views on its videos. In reference to how their team analyzes the algorithm, Wynne Levine reveals:
